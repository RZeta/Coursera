...
int changeValue() {
  ...
  int * intPtr2;
	int x=351;
intPtr1=&x;
printf("%p %p %i",intPtr1,intArray,x);
  *(intArray+5)=*intPtr1;
...
int withinArray(int * intArray, int size, int * ptr) {
  // TODO
  int * x=intArray+size-1 ;
printf("%p %p %p %i",intArray,ptr,x,(int)ptr-(int)intArray);
int  y=(int)ptr - (int)intArray;
int  z=(int)ptr-(int)x;
printf(" %i ",!!((y^z)>>31));
  return (!y | !z | !!((y^z)>>31)) ;
;
